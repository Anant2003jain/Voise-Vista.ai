...
def detect_gender(audio_segment, sample_rate):
    try:
        ...
        if weighted_pitch > 0:  # Only score if we detected pitch
            ...
            if mean_centroid < 2000:  # Male characteristic
                male_score += 1
            elif mean_centroid > 2500:  # Female characteristic
                female_score += 1
            
            # Formant scoring
            if formants < 0:  # Male characteristic
                male_score += 1
            elif formants > 10:  # Female characteristic
                female_score += 1
    ...
    for idx, segment in enumerate(translated_transcript):
        ...
        try:
            ...
            if result.reason == speechsdk.ResultReason.SynthesizingAudioCompleted:
                print(f"Speech synthesized for segment {idx}")
                segment['audio_file'] = audio_path
            else:
            ...
            print(f"Error synthesizing speech for segment {idx}: {e}")
            segment['audio_file'] = None
            continue
